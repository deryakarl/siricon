...
``zilver-node``
    ...
    start
        Detect local Apple Silicon hardware, register with a registry server,
        and start serving simulation jobs over HTTP.

    status
        Print a summary of the registry (online node count, backends, etc.).
...
``zilver-registry``
    ...
    start
        Bind a registry HTTP server to a port and serve indefinitely.

Typical two-Mac setup
---------------------
::
